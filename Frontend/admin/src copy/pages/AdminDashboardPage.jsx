...
function StatCard({ label, value, description }) {
  return (
    <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md hover:scale-105 active:scale-95 transition-all duration-300">
      ...
      <div className="mt-2 text-3xl font-bold text-[#38ADA9]">{value}</div>
...
function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6 pb-8 animate-fade-in-up">
      <div className="rounded-[32px] border border-slate-200/60 bg-white p-6 shadow-sm hover:shadow-md transition-all duration-300">
        <h1 className="text-3xl font-semibold text-slate-900">Dashboard Admin</h1>
        <p className="mt-2 text-sm text-slate-500">Ringkasan sistem dan performa keuangan aplikasi.</p>
      </div>

      {loading ? (
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 text-slate-600 shadow-sm">Memuat...</div>
      ) : error ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-all duration-300">
              ...
              <div className="space-y-4">
                {latestUsers.map((u) => (
                  <div key={u.id} className="flex items-center justify-between border-b border-slate-50 pb-3 last:border-0 last:pb-0">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-[10px] uppercase text-[#38ADA9] font-bold mt-0.5">{u.user_type || u.role}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-all duration-300">
              <h2 className="mb-4 text-base font-semibold text-slate-900">Transaksi Terbaru</h2>
              <div className="space-y-4">
          ...
          <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="space-y-4">
                {topCategories.map((c, idx) => (
                  <div key={idx} className="flex items-center justify-between border-b border-slate-50 pb-3 last:border-0 last:pb-0">
                    <div className="text-sm font-semibold text-slate-900">{c.category || 'Lainnya'}</div>
                    <div className="text-sm font-semibold text-slate-700">
                      Rp {Number(c.total).toLocaleString('id-ID')}
                      <span className="ml-2 text-xs font-normal text-slate-400">({c.count}x)</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-all duration-300">
              <h2 className="mb-4 text-base font-semibold text-slate-900">Status Sistem</h2>
              <div className="space-y-4">
                <div className="flex items-center justify-between rounded-2xl bg-slate-50 p-4">
                  <div className="text-sm font-semibold text-slate-700">Maintenance Mode</div>
                  <div className={`rounded-full px-3 py-1 text-xs font-bold ${systemStatus.maintenance_active ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                    {systemStatus.maintenance_active ? 'Aktif' : 'Nonaktif'}
                  </div>
                </div>
                <div className="flex items-center justify-between rounded-2xl bg-slate-50 p-4">
                  <div className="text-sm font-semibold text-slate-700">Error Logs Hari Ini</div>
