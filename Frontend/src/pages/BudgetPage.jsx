...
function BudgetPage({ transactions, budgets, setBudgets, userType }) {
  ...
  const handleSubmit = (e) => {
    ...
    const isKategoriTanpaDetail = [
      ...
      (isMasyarakatUser && selectedOperationalCategory === 'Kebutuhan Lainnya')
            ...
            {shouldShowCustomInput() && (
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  {userType === 'mahasiswa' ? 'Jelaskan Kebutuhan Lainnya' : 'Kebutuhan operasional'}
                </label>
                <input
                  type="text"
                ...
                <p className="mt-2 text-xs text-slate-500">
                  {userType === 'mahasiswa' ? 'Jelaskan jenis kebutuhan Anda' : 'Isi kebutuhan/jenis operasionalnya agar lebih spesifik.'}
                </p>
              </div>
            )}
