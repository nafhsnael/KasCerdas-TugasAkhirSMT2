  ...
  return (
    <section className={`rounded-[20px] bg-white shadow-sm border border-slate-200 ${compact ? 'p-4' : 'p-6'}`}>
      <div className="mb-4">
        ...
        <p className={`mt-2 text-sm text-slate-500 ${compact ? 'hidden' : ''}`}>Ringkasan cashflow untuk kebutuhan kuliah, kos, dan harian</p>
      </div>

      <div className="rounded-[16px] border border-slate-200 bg-slate-50/40 overflow-hidden">
        {[
          {
            label: 'Rata-rata pemasukan mahasiswa',
            value: formatRp(stats.incomeAvg),
            valueClass: 'text-emerald-700',
          },
          {
            label: 'Rata-rata pengeluaran mahasiswa',
