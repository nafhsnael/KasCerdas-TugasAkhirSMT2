...
function MasyarakatExpenseCompositionCard({ transactions, periodLabel, compact = false }) {
  const rows = useMemo(() => {
    ...
    >
      <div className={`mb-4 ${compact ? 'hidden' : ''}`}>
        ...
        <p className={`mt-2 text-sm text-slate-500 font-semibold ${compact ? 'hidden' : ''}`}>
          {periodLabel || 'Periode'} • Ringkasan berdasarkan total pengeluaran pada periode tersebut
        </p>
      </div>
