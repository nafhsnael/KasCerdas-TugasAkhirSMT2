...
function MonthlyCashflowTableCard({ transactions, periodLabel, compact = false }) {
  const stats = useMemo(() => {
    ...
    // Net cashflow = income - expense (based on averages)
    const netCashFlow = incomeAvg - expenseAvg

    const netCashHigher = incomeAvg >= expenseAvg

    // Highest/lowest net cashflow based on per-transaction cashflow
    // If a record is income -> netCash = +amount, if expense -> netCash = -amount
    // then pick the highest and lowest net cash values.
    const perTxNet = tx
      .map((t) => {
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <h3 className={`mt-2 font-semibold text-slate-900 ${compact ? 'text-lg' : 'text-xl'}`}>
              {periodLabel || 'Periode Terpilih'}
            </h3>
          </div>

        </div>
        <p className={`mt-2 text-sm text-slate-500 ${compact ? 'hidden' : ''}`}>
          Ringkasan kondisi arus kas pengguna berdasarkan periode yang dipilih
        </p>
      </div>
      ...
      <div className="rounded-[16px] border border-slate-200 bg-slate-50/40 overflow-hidden">
        {[
          ...
          >
            ...
            <div className={`font-semibold text-[16px] ${row.valueClass}`}>
              {row.value}
            </div>
          </div>
        ))}
...
export default MonthlyCashflowTableCard
