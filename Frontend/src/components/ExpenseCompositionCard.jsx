...
function ExpenseCompositionCard({ transactions, periodLabel, compact = false }) {
  ...
  // Kebutuhan contoh kategori utama; jika tidak ada data, tetap akan muncul dari sorted pertama.
  ...
  const orderedRows = (() => {
    ...
    // Tambahkan sisanya (selain 5 kategori contoh) supaya tidak kosong
  ...
  return (
    <section className={`rounded-[20px] border border-slate-200 bg-white shadow-sm ${compact ? 'p-4' : 'p-6'}`}>
      <div className={`mb-4 ${compact ? 'hidden' : ''}`}>
        <p className="text-sm uppercase tracking-[0.24em] text-slate-500 font-semibold">Komposisi Pengeluaran per Pos</p>
        <h3 className={`mt-2 font-bold text-slate-900 ${compact ? 'text-lg' : 'text-xl'}`}>
          Distribusi pengeluaran berdasarkan kategori utama
        </h3>
        <p className={`mt-2 text-sm text-slate-500 font-semibold ${compact ? 'hidden' : ''}`}>
          {periodLabel || 'Periode'} • Komposisi dihitung per kategori dari total pengeluaran pada periode tersebut
        </p>
      </div>
