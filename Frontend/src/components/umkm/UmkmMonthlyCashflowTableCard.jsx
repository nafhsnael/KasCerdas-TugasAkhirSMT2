...
function UmkmMonthlyCashflowTableCard({ transactions, periodLabel, compact = false }) {
  const stats = useMemo(() => {
  ...
  return (
    <section className={`rounded-[20px] bg-white shadow-sm border border-slate-200 ${compact ? 'p-4' : 'p-6'}`}>
      <div className="mb-4">
        ...
        <p className={`mt-2 text-sm text-slate-500 ${compact ? 'hidden' : ''}`}>Ringkasan cashflow untuk kebutuhan operasional UMKM pada periode yang dipilih</p>
      </div>
...
export default UmkmMonthlyCashflowTableCard
